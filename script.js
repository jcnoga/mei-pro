...
// ========== LEMBRETES COM SOM (NOVO) ==========
let shownReminders = new Set(); // Evita repetição na mesma sessão
...
function playNotificationSound() {
    try {
        ...
        if (audioCtx.state === 'suspended') {
            audioCtx.resume().then(() => playBeep(audioCtx)).catch(e => console.warn("Áudio bloqueado:", e));
        } else {
            playBeep(audioCtx);
        }
    } catch (e) {
...
function showReminderToast(message) {
    ...
    setTimeout(() => hideReminderToast(), 8000);
}
...
function loadFiscalReminders() {
    const listEl = document.getElementById('fiscal-reminders');
    if (!listEl) return;
    ...
    const formatted = dueDate.toLocaleDateString('pt-BR', options);
    listEl.innerHTML = `<li>🔔 DAS MEI vence em ${formatted}</li>`;
    // Você pode adicionar outros lembretes fiscais aqui (ex: declaração anual)
}
...
function checkReminders() {
    ...
    // 1. Lembrete fiscal: DAS dia 20
    const day = now.getDate();
    if (day === 20) {
        const key = `fiscal_das_${now.getFullYear()}_${now.getMonth()}`;
        if (!shownReminders.has(key)) {
            shownReminders.add(key);
            showReminderToast("⚠️ Hoje é o dia de pagar o DAS MEI! Não se esqueça.");
        }
    }

    // 2. Lembretes de agendamentos (data = hoje e horário futuro ou já passado?)
    const appointments = getUserData().appointments || [];
    appointments.forEach(app => {
        if (app.date === todayStr && app.status === 'agendado') {
            // Se não foi notificado ainda para este agendamento
            const key = `appt_${app.id}`;
            if (!shownReminders.has(key)) {
                // Verifica se o horário já passou ou está próximo
                const [hour, minute] = app.time.split(':').map(Number);
                const appDateTime = new Date();
                appDateTime.setHours(hour, minute, 0);
                const diffMinutes = (appDateTime - now) / 60000;
                // Notifica se estiver dentro da próxima hora (ou já passou há menos de 30 min)
                if (diffMinutes <= 60 && diffMinutes > -30) {
                    shownReminders.add(key);
                    showReminderToast(`⏰ Compromisso: ${app.title} com ${app.client_name} às ${app.time}`);
                }
            }
        }
    });
...
// Iniciar/parar o verificador de lembretes
function startReminderChecker() {
    if (reminderInterval) clearInterval(reminderInterval);
    reminderInterval = setInterval(checkReminders, 60000); // a cada minuto
    checkReminders(); // executa imediatamente também
}
...
function loginUser(user) {
    ...
    // Inicia o verificador de lembretes (NOVO)
    startReminderChecker();
}

function logout() {
    stopReminderChecker(); // (NOVO)
    appData.currentUser = null; sessionStorage.removeItem('mei_user_id'); location.reload();
}
...
function navTo(viewId) {
    ...
    if(viewId === 'dashboard') { 
        updateDashboard(); 
        loadFiscalReminders(); // Atualiza lista de lembretes fiscais (NOVO)
    }
